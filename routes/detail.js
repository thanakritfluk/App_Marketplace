...
async function find_is_exist(app_name,collection_name){
  ...
  try {
      ...
      if(res == null){
        ...
        if(collection_name=='Android_app_paid' || collection_name == 'Android_app_free'){
...
router.get('/:name', async function(req, res, next) {
  ...
  console.log(ios_url_available)
  res.render('detail', { detail: app_detail[0], ios_url:ios_url_available, android_url : android_url_available});
  app_detail = [];
});
